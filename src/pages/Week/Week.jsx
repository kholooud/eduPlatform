...
export default function Profile() {
  ...
  return !checkCode ? (
    ...
    <>
      {checkCode == 200 ? (
        ...
        >
          ...
          <Grid
            ...
            xs={12}
            md={3}
            sx={{ justifyContent: "center", alignContent: "start" }}
          >
            <WeekTab
              tabName={"المحاضرة"}
              View={"week"}
              current={current[0] == weekID ? "week" : ""}
              icon={<PersonIcon />}
            />
            <WeekTab
              tabName={"الواجب"}
          ...
          >
            <Outlet
              context={{ ...weekDetails, getLessonData: getLessonData }}
            />
          </Grid>
        </Grid>
      ) : (
      ...
      {checkCode == 403 ? (
        <Code_warning getLesson={getLessonData} lesson_id={weekID} />
      ) : (
        ""
      )}
      {checkCode == 404 ? <NotFound /> : ""}
      {checkCode == 401 ? <div>غير مسموح لك للدخول الحصة</div> : ""}
